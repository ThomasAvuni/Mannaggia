...
void AMannaggiaCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		// Moving
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &AMannaggiaCharacter::Move);
		EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AMannaggiaCharacter::Look);

		// Looking
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &AMannaggiaCharacter::Look);
	}
	else
	{
	...
	UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (Acubbo* Cubbo = Cast<Acubbo>(OtherActor))
	{
		if (ACubeCollectorGamemode* GameMode = Cast<ACubeCollectorGamemode>(UGameplayStatics::GetGameMode(GetWorld())))
		{
			GameMode->CubeCollected(Cubbo);
		}
		
		if (SoundAction)
		{
			UGameplayStatics::PlaySound2D (GetWorld(), SoundAction);
		}
...
void AMannaggiaCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AMannaggiaCharacter::DoLook(float Yaw, float Pitch)
{
	if (GetController() != nullptr)
	{
...
void AMannaggiaCharacter::DoJumpStart()
{
	// signal the character to jump
	Jump();
}

void AMannaggiaCharacter::DoJumpEnd()
{
